...
import numpy as np
import pandas as pd
# NOTE: matlab.engine is imported inside johcontest() instead of here, so that
# s_master can be imported without a working MATLAB Engine API installation.
# Only johcontest() needs MATLAB; nothing in Exercise 3 does.
...
def johcontest(Y,r,test,cons,model,lags,alpha):
    ...
    import matlab.engine  # requires the MATLAB Engine API for Python
...
#Companion matrix and stability check
#
# These two functions have been removed from this version of the master file:
#     comp(Beta, p)   - builds the companion matrix
#     stabVAR(A)      - returns the moduli of the eigenvalues
#
